...
@section('title', __('admin.audit_hash', ['id' => $auditLog->id]))
...
<div class="max-w-3xl space-y-5">
    ...
    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">
        {{ __('admin.audit_hash', ['id' => $auditLog->id]) }}
    </h1>

    <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-6 space-y-4">
        <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
            <div>
                <dt class="text-gray-500 dark:text-gray-400">{{ __('admin.action') }}</dt>
                <dd class="font-mono font-medium text-indigo-600 dark:text-indigo-400 mt-0.5">{{ $auditLog->action }}</dd>
            </div>
            <div>
                <dt class="text-gray-500 dark:text-gray-400">{{ __('admin.admin') }}</dt>
                <dd class="font-medium text-gray-900 dark:text-gray-100 mt-0.5">
                    {{ $auditLog->admin?->name ?? __('admin.system') }}
                </dd>
            </div>
            <div>
                <dt class="text-gray-500 dark:text-gray-400">{{ __('admin.entity') }}</dt>
                <dd class="font-medium text-gray-900 dark:text-gray-100 mt-0.5">
            ...
            <div class="sm:col-span-2">
                <dt class="text-gray-500 dark:text-gray-400">{{ __('admin.timestamp') }}</dt>
                <dd class="font-medium text-gray-900 dark:text-gray-100 mt-0.5">
