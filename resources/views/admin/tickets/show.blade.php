...
@section('title', __('admin.ticket_hash', ['id' => $ticket->id]))
...
<div class="space-y-6 max-w-4xl">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">
            {{ __('admin.ticket_hash', ['id' => $ticket->id]) }}
        </h1>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="md:col-span-2 space-y-4">
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-5">
                ...
                <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                    <div>
                        <dt class="text-gray-500 dark:text-gray-400">{{ __('admin.customer') }}</dt>
                        ...
                        <dd class="text-xs text-gray-400">{{ $ticket->user?->phone ?? $ticket->user?->email }}</dd>
                    </div>
                    <div>
                        <dt class="text-gray-500 dark:text-gray-400">{{ __('admin.lottery') }}</dt>
                        <dd class="font-medium text-gray-900 dark:text-gray-100 mt-0.5">
                            {{ $ticket->lottery?->name }}
                        </dd>
                        <dd class="text-xs text-gray-400">
                            {{ __('admin.draw_date') }}: {{ $ticket->lottery?->draw_date?->format('M d, Y') }}
                        </dd>
                    </div>
                    <div>
                        <dt class="text-gray-500 dark:text-gray-400">{{ __('admin.transaction_id') }}</dt>
                        <dd class="font-mono font-medium text-gray-900 dark:text-gray-100 mt-0.5">
                            {{ $ticket->transaction_id }}
                        </dd>
                    </div>
                    <div>
                        <dt class="text-gray-500 dark:text-gray-400">{{ __('admin.payment_method') }}</dt>
                        <dd class="font-medium text-gray-900 dark:text-gray-100 mt-0.5">
                            {{ $ticket->payment_method }}
                        </dd>
                    </div>
                    <div>
                        <dt class="text-gray-500 dark:text-gray-400">{{ __('admin.unit_price') }}</dt>
                        <dd class="font-medium text-gray-900 dark:text-gray-100 mt-0.5">
                            ETB {{ number_format($ticket->ticket_price, 2) }}
                        </dd>
                    </div>
                    <div>
                        <dt class="text-gray-500 dark:text-gray-400">{{ __('admin.quantity') }}</dt>
                        <dd class="font-medium text-gray-900 dark:text-gray-100 mt-0.5">
                            {{ $ticket->quantity ?? 1 }} {{ __('admin.tickets_label') }}
                        </dd>
                    </div>
                    <div class="sm:col-span-2">
                        <dt class="text-gray-500 dark:text-gray-400">{{ __('admin.total_paid') }}</dt>
                        <dd class="mt-1">
                    ...
                    <div>
                        <dt class="text-gray-500 dark:text-gray-400">{{ __('admin.submitted') }}</dt>
                        <dd class="font-medium text-gray-900 dark:text-gray-100 mt-0.5">
                    ...
                    @if($ticket->reviewer)
                    <div>
                        <dt class="text-gray-500 dark:text-gray-400">{{ __('admin.reviewed_by') }}</dt>
                        <dd class="font-medium text-gray-900 dark:text-gray-100 mt-0.5">
                    ...
                    <div class="sm:col-span-2">
                        <dt class="text-gray-500 dark:text-gray-400">{{ __('admin.lottery_numbers') }}</dt>
                    ...
                    <div class="sm:col-span-2">
                        <dt class="text-gray-500 dark:text-gray-400">{{ __('admin.rejection_reason') }}</dt>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-5 space-y-4">
                <h2 class="font-semibold text-gray-700 dark:text-gray-300 mb-2">
                    {{ __('admin.review_actions') }}
                </h2>

                @can('approve', $ticket)
                <form method="POST" action="{{ route('admin.tickets.approve', $ticket) }}"
                      onsubmit="return confirm('{{ __('tickets.approve_confirm') }}')">
                    @csrf
                    <button type="submit"
                        ...
                        <form method="POST" action="{{ route('admin.tickets.reject', $ticket) }}">
                            ...
                            <div class="space-y-3">
                                <textarea name="reason" rows="3" required
                                          placeholder="{{ __('tickets.rejection_reason_placeholder') }}"
        ...
        <div class="space-y-4">
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-5">
                <h2 class="font-semibold text-gray-700 dark:text-gray-300 mb-3">
                    {{ __('admin.payment_screenshot') }}
                </h2>
                @if($ticket->screenshot_path)
                <a href="{{ route('admin.tickets.screenshot', $ticket) }}" target="_blank">
                    <img src="{{ route('admin.tickets.screenshot', $ticket) }}"
                         alt="{{ __('admin.payment_screenshot') }}"
                ...
                <div class="text-center py-8 text-gray-400">
                    ...
                    <p class="text-sm">{{ __('admin.no_screenshot') }}</p>
                </div>
                @endif
            </div>
        </div>
